  ...
  methods: {
    fav(index) {
      const result = this.shares[index].like.some((value) => {
        return value.user_id == this.$router.state.user.id;
      });
      if (result) {
        this.shares[index] .like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            axios({
              method: "delete",
              url: "https://salty-waters-72803.herokuapp.com/api/like",
              data: {
        ...
        axios
        .post("https://salty-waters-72803.herokuapp.com/api/like", {
          share_id: this.shares[index].item.id,
          user_id: this.$store.state.user.id,
        })
        .then((response) => {
    ...
    del(index) {
      axios
      .delete(
        "https://salty-waters-72803.herokuapp.com/api/shares/" +
